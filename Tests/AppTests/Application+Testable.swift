...
extension Application {
  //MARK: - Create the testing database environment
  ...
  //MARK: - Reset the testing database
  static func reset() throws {
    let revertEnvironment = ["Partage", "revert", "--all", "-y"]
    try Application.testable(envArgs: revertEnvironment).asyncRun().wait()
    let migrateEnvironment = ["Partage", "migrate", "-y"]
    try Application.testable(envArgs: migrateEnvironment).asyncRun().wait()
  }
  
  //MARK: - Sends a request to a path and returns a Response
  func sendRequest<T>(to path: String, method: HTTPMethod, headers: HTTPHeaders = .init(), body: T? = nil, loggedInRequest: Bool = false, loggedInUser: User? = nil) throws -> Response where T: Content {
    ...
    if (loggedInRequest || loggedInUser != nil) {
      ...
      let credentials = BasicAuthorization(username: userEmail, password: "password")
  ...
  //MARK: - Sends a request to a path without a body
  ...
  //MARK: - Sends a request to a path and accepts a generic Content Type
  ...
  //MARK: - To accept a Content type and Decodable type to get a response to a request
  ...
  //MARK: - To accept a Decodable type to get a response to a request without providing a body
